...
class Dataset:
    ...
    def _read_grib_with_iris(self,field_list,ensemble):
        data = load_grib(self._input_grib_files,field_list=field_list,ensemble=ensemble)
        return data
    ...
    def calculate_isobaric_frontogenesis(self,pressure_level):
        ...
        if 'frontogenesis_isobaric' in self.data.variables:
            self.data['frontogenesis_isobaric'].loc[dict(pressure=[pressure_level])] = fgen
        else:
            self.data['frontogenesis_isobaric'] = fgen
