...
def main():
    ...
    ds.calculate_isobaric_frontogenesis(args.pressure_level)
    for valid_time in ds.valid_time:

        GH = ds.data.geopotential_height_isobaric.sel(
            time = valid_time,
            pressure = args.pressure_level
        )
        FGEN = ds.data.frontogenesis_isobaric.sel(
            time = valid_time,
            pressure = args.pressure_level
        )
        print(GH)
        print(FGEN)
        ds.plotter.plot_isobaric_fgen_heights(
            valid_time,
            GH,
            FGEN
        )
